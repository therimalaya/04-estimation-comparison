...
\begin{equation}
  ...
  (\#eq:min-err)
\end{equation}
The estimation error $\widehat{\mathcal{EE}}_\circ$ for every methods, design and response corresponding to $l_\circ$ component, computed as \@ref(eq:min-err), is then regarded as _error dataset_ in the subsequent analysis. Let $\mathbf{u}_{8000\times4}=(u_j)$ for $j = 1, \ldots 4$ be the outcome variables measuring the estimation error corresponding to the response $j$ in the context of this dataset. Further, let the number of components that result in minimum estimation error in each replication be $l_\circ$ computed as \@ref(eq:min-comp) will be considred as _component dataset_.
